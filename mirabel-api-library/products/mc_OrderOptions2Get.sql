/*
 =============================================
 API Documentation-mc_OrderOptions2Get
 Description:    Retrieves advertisement color information with optional filtering
 ...
 Example Usage:
 EXEC [mc_OrderOptions2Get] @RateCardID = '', @ProductID = '', @PageNumber = '1'
 EXEC [mc_OrderOptions2Get] @RateCardID = '7', @ProductID = '', @PageNumber = '1'
 EXEC [mc_OrderOptions2Get] @RateCardID = '', @ProductID = '25', @PageNumber = '1'
 =============================================

*/
CREATE OR ALTER PROCEDURE [dbo].[mc_OrderOptions2Get]
(
...
AS
BEGIN
    -- Returns advertisement color options with the correct label
    SET NOCOUNT ON;
    ...
    BEGIN TRY
        ...
        SELECT DISTINCT 
            a.ColorID as ID,
            a.ColorCode AS [Value],
            e.SubProductType AS ProductType,
            f.Label
        FROM gsAdColor a
        LEFT OUTER JOIN tblratecard2adcolor c ON a.ColorID = c.AdColorID
        ...
        LEFT JOIN tblProductTypeFieldLabel f ON e.subproducttypeID = f.subproducttypeID 
            AND producttypefieldcode = 'AdColor'
        WHERE e.isenabled = 1
