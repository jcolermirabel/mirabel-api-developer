/*
 =============================================
 API Documentation--mc_OrderOptions3Get
 Description:    Retrieves advertisement frequency information with optional filtering
 ...
 Success: Table with columns
   - ID (int) - Frequency ID
   - Value (nvarchar) - Frequency value
 ...
 Example Usage:
 EXEC [mc_OrderOptions3Get] @RateCardID = '', @ProductID = '', @PageNumber = '1'
 EXEC [mc_OrderOptions3Get] @RateCardID = '7', @ProductID = '', @PageNumber = '1'
 EXEC [mc_OrderOptions3Get] @RateCardID = '', @ProductID = '25', @PageNumber = '1'
 =============================================
*/
CREATE OR ALTER PROCEDURE [dbo].[mc_OrderOptions3Get]
(
...
AS
BEGIN
    -- Returns advertisement frequency options with the correct label
    SET NOCOUNT ON;
    ...
    BEGIN TRY
        ...
        SELECT DISTINCT 
            a.FrequencyID AS ID,
            a.Frequency AS [Value],
            e.SubProductType AS ProductType,
            f.Label
        FROM gsAdFrequency a
        LEFT OUTER JOIN tblratecard2adfrequency c ON a.frequencyID = c.frequencyID
